...
const ChatWidget: React.FC = () => {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    { role: 'model', text: 'Chào bạn! 👋 Mình là Gia sư Tiếng Anh AI.\nBạn có thể:\n- 🗣️ Luyện hội thoại (role-play) với mình\n- 📝 Hỏi ngữ pháp, từ vựng, cách diễn đạt\n- 📸 Gửi ảnh đoạn văn tiếng Anh để mình dịch & phân tích\n- 💡 Hỏi mẹo phát âm, idioms, và văn hóa giao tiếp\n\nLet\'s start! 🚀' }
  ]);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setMessages([...newHistory, { role: 'model', text: 'Xin lỗi, có chút trục trặc kỹ thuật. Bạn thử lại sau nhé!' }]);
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <div className="fixed bottom-6 right-6 z-50 w-[90vw] md:w-[400px] h-[550px] max-h-[85vh] bg-white rounded-3xl shadow-2xl flex flex-col border border-gray-100 animate-fade-in overflow-hidden ring-1 ring-black/5">
          ...
          <div className="p-4 bg-gradient-to-r from-blue-600 to-indigo-600 text-white flex justify-between items-center shadow-md">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-bold text-lg">Thầy Tiếng Anh AI</h3>
                <div className="flex items-center gap-1.5 opacity-90">
                  <span className="w-2 h-2 bg-green-400 rounded-full animate-pulse"></span>
                  <span className="text-xs font-medium">Sẵn sàng luyện tiếng Anh</span>
                </div>
              </div>
            </div>
            <button
          ...
          {selectedImage && (
            <div className="px-4 py-2 bg-gray-50 border-t border-gray-100 flex items-start gap-2 animate-fade-in">
              ...
              <div className="text-xs text-gray-500 italic mt-1">
                Đã chọn ảnh. Nhấn gửi để phân tích nhé!
              </div>
            </div>
          )}
          ...
          <div className="p-4 bg-white border-t border-gray-100">
            <div className="flex items-center gap-2 bg-gray-50 p-2 rounded-2xl border border-gray-200 focus-within:border-blue-400 focus-within:ring-2 focus-within:ring-blue-100 transition-all">
              ...
              <input
                ...
                placeholder={selectedImage ? "Thêm ghi chú..." : "Ask me anything in English..."}
